...
export default function WorkersPage() {
  ...
  const queryString = searchParams.toString()
    ...
    : "";
  const { data: workersResponse, isLoading: isLoadingWorkers } =
    useGetFilteredWorkersQuery(queryString);
  ...
  const delayDebounce = setTimeout(() => {
    ...
    if (minPriceInput.trim()) {
      newParams.set("price[gte]", minPriceInput);
    } else {
      newParams.delete("price[gte]");
    }

    if (maxPriceInput.trim()) {
      newParams.set("price[lte]", maxPriceInput);
    } else {
    ...
    >
      <div className="md:hidden flex justify-between items-center bg-white p-4 rounded-xl border border-gray-100">
        <button
          ...
          className="flex items-center gap-2 bg-orange-500 text-white px-4 py-2 rounded-lg text-sm font-bold"
        >
      ...
      <aside className="hidden md:block w-1/4 bg-white p-6 rounded-2xl border border-gray-100 shadow-sm h-fit sticky top-24">
        <h3 className="text-lg font-bold text-gray-900 mb-4 border-b pb-3 flex items-center gap-2">
          <SlidersHorizontal size={18} className="text-orange-500" /> تصفية
          النتائج
        </h3>
        <FiltersSidebar
      ...
      >
        ...
        <div className="fixed inset-0 flex justify-end">
          <DialogPanel className="w-full max-w-xs bg-white p-6 shadow-xl flex flex-col h-full justify-between overflow-y-auto">
            ...
            <button
              onClick={() => setIsMobileFiltersOpen(false)}
              className="w-full bg-orange-500 text-white py-3 rounded-xl font-bold mt-6"
            >
