...
export default function FiltersSidebar({ categories, isLoadingCategories, searchParams, handleFilterChange }) {
  ...
  return (
    <div className="space-y-6 text-right">
      ...
      <div className="border-t pt-4">
        <h4 className="font-bold text-gray-700 mb-3 text-sm">نطاق السعر (ج.م)</h4>
      ...
      <div className="border-t pt-4">
        ...
        <div className="space-y-1">
          {[4, 3, 2].map((stars) => (
            <button
              key={stars}
              onClick={() => handleFilterChange('rating[gte]', stars.toString())}
              className={`w-full flex items-center justify-between text-xs px-3 py-1.5 rounded-lg transition ${
                currentRating === stars.toString() ? 'bg-orange-50 text-orange-600 font-bold' : 'text-gray-600 hover:bg-gray-50'
              }`}
            >
              <span className="flex items-center gap-1">{stars} نجوم وأعلى</span>
              <div className="flex gap-0.5">
                {[...Array(5)].map((_, i) => (
                  <Star key={i} size={10} className={`${i < stars ? 'fill-amber-400 text-amber-400' : 'text-gray-200'}`} />
                ))}
              </div>
            </button>
          ))}
      ...
      <div className="border-t pt-4">
        <label className="flex items-center gap-2 cursor-pointer select-none">
          <input
            ...
            onChange={(e) => handleFilterChange('isOnline', e.target.checked ? 'true' : '')}
            className="w-4 h-4 text-orange-500 accent-orange-500"
          />
          <span className="text-xs font-bold text-gray-700">المتاحين أونلاين فقط</span>
        </label>
      </div>
    </div>
  );
